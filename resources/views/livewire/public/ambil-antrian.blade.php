<div class="min-h-screen bg-gradient-to-br from-teal-50 to-gray-100 py-4 sm:py-6 md:py-8 px-3 sm:px-4">
    <div class="w-full max-w-4xl mx-auto">
        @if($submitted)
            {{-- Success View - Mobile Optimized --}}
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 text-center border border-teal-100">
                <div class="w-16 h-16 sm:w-20 md:w-24 sm:h-20 md:h-24 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-lg">
                ...
                <h2 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-2">Pendaftaran Berhasil!</h2>
                <p class="text-gray-600 mb-4 sm:mb-6 md:mb-8 text-base sm:text-lg">Antrian kunjungan Anda telah terdaftar.</p>
                
                <div class="bg-gradient-to-r from-teal-50 to-teal-100 border-2 border-teal-300 rounded-lg sm:rounded-xl p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 md:mb-8">
                    <p class="text-xs sm:text-sm text-gray-600 mb-2 uppercase tracking-wide font-semibold">Nomor Antrian</p>
                    <p class="text-3xl sm:text-4xl md:text-5xl font-black text-teal-700 tracking-wider">{{ $queueData->nomor_antrian }}</p>
                    <div class="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-teal-200">
                        <p class="text-xs sm:text-sm text-gray-600">Kode Booking</p>
                        <p class="text-xl sm:text-2xl font-bold text-teal-600 font-mono">{{ $queueData->kode_booking }}</p>
                    </div>
                </div>
                
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                    <a href="{{ route('download-pdf', $queueData->id) }}" 
                       class="inline-flex items-center justify-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 bg-teal-600 text-white rounded-lg sm:rounded-xl hover:bg-teal-700 shadow-lg transition-all transform hover:scale-105 font-semibold text-base sm:text-lg">
                    ...
                    <a href="{{ route('home') }}" 
                       class="inline-flex items-center justify-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 border-2 border-gray-300 text-gray-700 rounded-lg sm:rounded-xl hover:bg-gray-50 transition-all font-semibold text-base sm:text-lg">
        ...
        @else
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-xl overflow-hidden border border-gray-200">
                <div class="p-4 sm:p-6 md:p-8 border-b border-gray-200 bg-gradient-to-r from-teal-600 to-teal-700 text-white">
                    <h2 class="text-xl sm:text-2xl md:text-3xl font-bold mb-1 sm:mb-2">Ambil Antrian Kunjungan</h2>
                    <p class="text-teal-100 text-sm sm:text-base">Langkah {{ $currentStep }} dari {{ $totalSteps }}</p>
                </div>
                        ...
                        @foreach([1, 2, 3] as $step)
                            <div class="flex items-center">
                                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs sm:text-sm font-bold shadow-md
                                    {{ $currentStep == $step ? 'bg-teal-600 text-white ring-2 sm:ring-4 ring-teal-200' : '' }}
                                ...
                                @if($step < 3)
                                    <div class="w-8 sm:w-16 md:w-32 h-1.5 sm:h-2 mx-1 sm:mx-2 rounded-full {{ $currentStep > $step ? 'bg-green-500' : 'bg-gray-300' }}"></div>
                                @endif
                            </div>
                        @endforeach
                    </div>
                ...
                <form wire:submit.prevent="{{ $currentStep === 3 ? 'submit' : 'nextStep' }}" class="p-3 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
                    ...
                    @if($currentStep === 1)
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center gap-2 mb-6">
                                ...
                                <h3 class="text-xl font-bold text-gray-900">Data Pengunjung Utama</h3>
                            </div>
                                ...
                                <div class="md:col-span-2 space-y-2">
                                    ...
                                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 bg-gray-50 hover:bg-gray-100 transition-colors">
                                        <input type="file" wire:model="foto_identitas" accept="image/jpeg,image/jpg,image/png"
                                               class="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-2 file:border-teal-500 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
                                        <p class="text-xs text-gray-500 mt-2">Format: JPG, JPEG, PNG. Maksimal 2MB</p>
                                    </div>
                                    <div wire:loading wire:target="foto_identitas" class="text-sm text-teal-600 font-medium flex items-center gap-2">
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-center gap-2">
                                    ...
                                    <h3 class="text-xl font-bold text-gray-900">Data Pengikut (Maksimal 10 orang)</h3>
                                </div>
                                @if(count($followers) < 10)
                                    <button type="button" wire:click="addFollower" 
                                            class="px-4 py-2 bg-teal-100 text-teal-700 rounded-lg hover:bg-teal-200 font-semibold transition-colors flex items-center gap-2">
                                ...
                                <div class="bg-gray-50 border-2 border-gray-200 rounded-xl p-5 space-y-4 relative">
                                    <div class="flex justify-between items-center border-b border-gray-200 pb-3">
                                        <span class="font-bold text-gray-900 flex items-center gap-2">
                                            <span class="w-6 h-6 bg-teal-600 text-white rounded-full flex items-center justify-center text-sm">{{ $index + 1 }}</span>
                                            Pengikut
                                        </span>
                            ...
                            @empty
                                <div class="text-center py-8 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl">
                                    ...
                                    <button type="button" wire:click="addFollower" 
                                            class="px-6 py-3 bg-teal-600 text-white rounded-lg hover:bg-teal-700 font-semibold transition-colors">
                                        + Tambah Pengikut
                                    </button>
                                </div>
                            @endforelse
                    ...
                    <div class="flex flex-col-reverse sm:flex-row justify-between gap-3 sm:gap-4 pt-6 sm:pt-8 border-t-2 border-gray-200">
                        ...
                        @else
                            <a href="{{ route('home') }}" 
                               class="w-full sm:w-auto px-4 sm:px-6 md:px-8 py-3 sm:py-4 border-2 border-gray-300 text-gray-700 rounded-lg sm:rounded-xl hover:bg-gray-50 font-semibold transition-colors text-center">
                                <span class="truncate">Batal</span>
                            </a>
                        @endif
                        
                        <button type="submit" 
                                class="w-full sm:w-auto px-4 sm:px-6 md:px-8 py-3 sm:py-4 bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-lg sm:rounded-xl hover:from-teal-700 hover:to-teal-800 font-semibold transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-2 min-h-[48px]">
                            @if($currentStep === 3)
                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
